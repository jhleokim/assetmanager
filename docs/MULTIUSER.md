...
> ⚠️ **이 문서의 저장 설계는 [`DATA-PRIVACY.md`](DATA-PRIVACY.md)에서 개정되었다.**
> 아래 §2는 자산 데이터를 D1에 **평문으로** 두는 것을 전제했으나, 검토 결과
> **로컬 우선 + 종단간 암호화 동기화**로 바꾸기로 했다. 스키마·인증·비용 부분은
> 그대로 유효하고, `assets` 테이블만 가구별 암호문 blob으로 대체된다.
